...
def add_vote(vote_label_pool, point_idx, pred_label, weight):
    ...
    for b in range(B):
        for n in range(N):
            if weight[b, n] != 0 and not np.isinf(weight[b, n]):#检查当前点的权重是否是无穷大
...
def main(args):
    ...
    with torch.no_grad():
        scene_id = TEST_DATASET_WHOLE_SCENE.file_list #每个点云文件
        scene_id = [x[:-4] for x in scene_id] #去掉文件最后四个字符  .npy
        num_batches = len(TEST_DATASET_WHOLE_SCENE)

        total_seen_class = [0 for _ in range(NUM_CLASSES)] #初始化一个大小为 NUM_CLASSES 的列表，所有元素都为 0
        ...
        for batch_idx in range(num_batches):
            ...
            for _ in tqdm(range(args.num_votes), total=args.num_votes):
                ...
                for sbatch in range(s_batch_num):
                    ...
                    real_batch_size = end_idx - start_idx #计算当前小批次的实际大小
                    ...
                    '''vote_label_pool = np.array([
                        [10, 5, 2, 1],  # 第1个点投给类别0 10票，类别1 5票，类别2 2票，类别3 1票
                        [2, 8, 6, 4],   # 第2个点投给类别0 2票，类别1 8票，类别2 6票，类别3 4票
                        [0, 1, 10, 3]   # 第3个点投给类别0 0票，类别1 1票，类别2 10票，类别3 3票
                    ])'''
            ...
            #每个点的预测标签（即 pred_label）保存到一个文本文件中
