...
#enumerate() 将一个可迭代对象（如列表）g_classes 转换为一个迭代器，每次返回一个元组 (index, value)，其中 index 是元素的索引，value 是该位置的元素。
#cls: i 字典格式
g_class2label = {cls: i for i,cls in enumerate(g_classes)}
g_class2color = {'excavator':	[0,255,0],
                 'ground':	[0,0,255],
                 'soil':	[255,0,0]}
...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    """
    points_list = []
    #'*.txt'：是一个通配符模式，表示该目录下的所有扩展名为 .txt 的文件。* 代表可以匹配任意多个字符
    #用于根据通配符 pattern 查找匹配的文件，返回的是一个文件路径列表（这里返回的是每一个点的路径）
    #glob.glob() 用于匹配文件路径名，它会返回一个符合指定模式的文件路径列表
    for f in glob.glob(os.path.join(anno_path, '*.pcd')):
        #通过 os.path.basename(f) 可以提取出该路径的最后部分（即文件名）
        #类别名
        # cls = os.path.basename(f)
        cls = os.path.splitext(os.path.basename(f))[0]
        print(cls)
        if cls not in g_classes: # note: in some room there is 'staris' class..
            cls = 'ground'#杂项
        #用于从一个文本文件中读取数据，并将其以数组的形式返回，每一个点的信息
        # points = np.loadtxt(f)
    ...
    #通过减去 xyz_min，将点云的最小坐标平移到原点附近，使得所有坐标在 (0, ∞) 的范围内。
    # 这是一种简单的 中心化 操作，可以保证数据的几何中心或边界位于原点，减小数值范围
